...
func _build_world() -> void:
	...
	# Filet de sécurité générique : toute map sans DirectionalLight3D propre
	# (Arena1v1.tscn, ArenaLabyrinth.tscn, ...) reçoit ce soleil de secours.
	# Demande explicite : le rendu du Donjon sans lui "fait bizarre", donc
	# il reste actif aussi en Co-op malgré la tentative précédente de le
	# retirer pour ce mode.
...
func _build_hud() -> void:
	...
	# Profil façon "carte de héros" compétitive, habillé du cadre ATH dédié
	# au héros (assets/ath_design/<héros>/<héros>_ath.png, fourni par
	...
	if ResourceLoader.exists(ath_path):
		...
		ath_frame.name = "AthFrame"
		ath_frame.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
		ath_frame.texture = load(ath_path) as Texture2D
		ath_frame.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
		ath_frame.stretch_mode = TextureRect.STRETCH_SCALE
		ath_frame.mouse_filter = Control.MOUSE_FILTER_IGNORE
